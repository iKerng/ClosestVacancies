...
def get_regions(areas=dict(), reload=0):
    ...
    if (not len(areas) == 0) and reload == 1:
        pd.DataFrame([x for x in areas])[['id', 'parent_id', 'name']].\
            to_sql('region', con=connect, if_exists='replace', index=False)
        ls_cities = []
        for region in areas:
            for city in region.get('areas'):
                ls_cities.append(city)
        pd.DataFrame(ls_cities)[['id', 'parent_id', 'name']].\
            to_sql('cities', con=connect, if_exists='replace', index=False)
    connect.close()
...
def get_dictionaries(dictionaries=dict(), reload=0):
    ...
    if (not len(dictionaries) == 0) and reload == 1:
        for name_dict in ls_dicts:
            pd.DataFrame(dictionaries.get(name_dict)).\
                to_sql(name=name_dict, con=connect, if_exists='replace', index=False)
    connect.close()
